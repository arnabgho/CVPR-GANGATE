...
% \paragraph{Variants of Soft-Gating (Proposed)}
\rz{May need to be more clear about notation on scalars vs vectors vs tensor. May need to cleanly separate out the 4 variants (block/channel, bias/no bias)...or those could possibly just be listed in experiments.}

The methods above do not fundamentally modify the structure of the network. In a residual network, an input feature tensor $x_l$ $l$ is modified by function $x_{l+1} = x_l+H_l(x_l)$. Changes in resolution are obtained by upsampling or downsampling before the residual block.
...
\paragraph{Adaptive Instance Normalization} Adding a bias to channel-wise gating bears strong resemblance to the Adaptive Instance Normalization (AdaIN) layer~\cite{huang2017arbitrary}. The layer has previously been used to great success in style transfer~\cite{huang2017arbitrary} and multimodal image-to-image translation~\cite{XX}. \rz{sentence summarizing why or how}
...
\begin{equation}
x + \mathbf{\alpha} \odot \text{IN} (H(x)) + \beta, \text{where IN is instance normalization}
\end{equation}

In AdaIN, $\alpha$ is not constrained between $[0,1]$, and an instance normalization~\cite{XX} is applied
